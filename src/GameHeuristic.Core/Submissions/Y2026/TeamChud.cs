...
public class TeamChud : IHeuristic
{
    public string Name => "Team Chud - Beau & Alfred";
    ...
    public double Evaluate(Player[,] board, Player player)
    {
        ...
        for (int r = 0; r < Board.Rows; r++)
        {
            for (int c = 0; c < Board.Columns; c++)
            {
                if (board[r, c] == player)
                {
                    score += Math.Max(0, centerCol - Math.Abs(c - centerCol));
                }
            }
        }
    ...
    private double EvaluateWindow(Player p1, Player p2, Player p3, Player p4, Player player)
    {
        ...
        foreach (Player p in new[] { p1, p2, p3, p4 })
        {
